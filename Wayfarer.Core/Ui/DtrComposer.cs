...
/// <item><description><b>Nothing engaged</b> — the plugin's bare name.</description></item>
/// </list></summary>
public static class DtrComposer
{
    public static DtrText Compose(DtrInputs inputs)
    {
        ...
        {
            DtrNextStep.Teleport => Aetheryte("Teleport", inputs.StepTarget),
            DtrNextStep.Aethernet => Aetheryte("Aethernet", inputs.StepTarget),
            _ => Walking(inputs),
        };
    }
    ...
    private static DtrText Aetheryte(string verb, string? target) =>
        new(target is { Length: > 0 } name ? $"{verb}: {name}" : verb, DtrGlyph.Aetheryte);

    /// <summary>Walking there: the progress through the plan and the distance left, and no glyph.
    /// Either half may be missing — an objective with no ordered route has no stop count, and a
    /// target with no coordinates has no distance — so the parts are joined rather than
    /// templated.</summary>
    private static DtrText Walking(DtrInputs inputs)
    {
        ...
        return text is { Length: > 0 } words
            ? new DtrText(words, DtrGlyph.None)
            : DtrText.Wayfarer;
    }

    private static string? Progress(DtrInputs inputs) =>
        inputs.RouteStop is { } stop && inputs.RouteTotal is { } total ? $"{stop}/{total}" : null;
}
