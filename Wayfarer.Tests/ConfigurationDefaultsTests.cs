...
/// the map flag — is on, and the settings are where you go to have less rather than to have anything
/// at all.
///
/// <para>Read out of the source file rather than out of the object, because <c>Configuration</c>
/// lives in the plugin assembly, which references Dalamud and cannot be loaded in a plain test host.
/// A regex over the declarations is cruder than reflection, and it catches exactly the drift this
/// exists for — somebody dropping <c>= true</c> off one of these while adding something else
/// nearby.</para>
///
...
/// the player's own value back before the declared default matters, and these properties are older
/// than the Version field itself. It cannot be reached from here; see <c>Configuration.Migrate</c>,
/// where the reasoning is written down beside the code it is about.</para></summary>
public partial class ConfigurationDefaultsTests
{
    /// <summary>The surfaces a first run must have switched on. Each of them is the only way a
    /// player who has not opened the window learns that part of the plugin exists, which is why none
    /// of them waits to be asked for: <c>MarkObjectiveWithMapFlag</c> is the guidance the game itself
    /// draws.</summary>
    public static TheoryData<string> OnByDefault =>
        ["MarkObjectiveWithMapFlag"];
    ...
    {
        // The core of it, held separately from the surfaces above so that a change of mind about them
        // can never reach the arrow. A first run that has to switch the arrow on has missed the point
        // of the plugin.
        var source = ConfigurationSource();
