...
/// <summary>The geometry proof: no part of the readout may be drawn outside the box it belongs to,
/// at any HUD scale.
///
/// <para>These exist because the field report was "everything is bleeding out of bounds of boxes",
/// and that was not a rendering accident — it was arithmetic. Nothing that can be measured should be
/// discovered by looking at it.</para>
///
/// <para><b>The scales are swept for real.</b> The readout's width is the banner's own and never
/// resizes, so the thing that varies is the scale itself — and unlike a window, the readout's own
/// text-size preference multiplies on top of the HUD's, so the two do not cancel. Every proof below
/// therefore runs at 100%, 150% and 200% against the worst content the composer can actually
/// emit.</para></summary>
