...
///
/// <para>A module that needs the framework thread for something marshals there itself, as the
/// toolkit's calls do; the host awaits each module plainly so an unload can never wait on a tick.
/// A module that throws while coming up is logged and left down; one that throws while going down
/// is logged and treated as down. Neither stops the other modules.</para></summary>
internal sealed class ModuleHost(IEnumerable<IModule> modules, IConfigStore configs, IFaultLog faults) : IModuleHost, IAsyncDisposable
{
    ...
    private async Task BringUpAsync(IModule module)
    {
        ...
        {
            faults.Record($"starting the {module.Name} module", ex);
        }
    }
    ...
    private async Task TakeDownAsync(IModule module)
    {
        ...
        {
            faults.Record($"stopping the {module.Name} module", ex);
        }
    }
}
