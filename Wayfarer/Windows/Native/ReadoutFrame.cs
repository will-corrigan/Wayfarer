...
// Position is deliberately absent: where the readout goes depends on how big it turns out to be,
// which is not known until it has been laid out, so the hosts ask ReadoutPlacement afterwards.
...
/// <param name="ArrowIcon">Which colour to draw the arrow in. Read every frame so the setting
/// applies without a reload.</param>
...
/// <param name="MoveMode">Whether the player has the readout in "move me" mode, in which the game's
/// own HUD-Layout handle is drawn over it and it can be dragged with the mouse.</param>
